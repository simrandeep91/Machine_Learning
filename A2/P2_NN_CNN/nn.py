...
from util import LoadData, Load, Save, DisplayPlot, ShowMeans
import sys
import numpy as np
...
def AffineBackward(grad_y, x, w):
    ...
    #grad_x = w.dot(grad_y.T);
    #grad_w = grad_y.T.dot(x);
    grad_x = grad_y.dot(w.T)
    grad_w = x.T.dot(grad_y)
    #grad_b = np.ones((grad_y.shape[0],1));
    #grad_b = (np.ones((x.shape[0],1))).T.dot(grad_y)
    
    grad_b = grad_y.T.dot(np.ones((x.shape[0])))
    #grad_b = np.copy(grad_y)
    #grad_b = np.sum(grad_y, axis=1).reshape(-1, 1)
    #print(grad_b)
    #print(grad_b.shape)
    #grad_b = np.sum(grad_y,np.ones((grad_y.shape[0],1)));
...
def NNUpdate(model, eps, momentum):
    ...
    Args:
        ...
        momentum: Momentum.
    """
    model['vW1'] = -(eps*model['dE_dW1']) - momentum*model['vW1']
    model['vW2'] = -(eps*model['dE_dW2']) - momentum*model['vW2']
    model['vW3'] = -(eps*model['dE_dW3']) - momentum*model['vW3']
    model['vb1'] = -(eps*model['dE_db1']) - momentum*model['vb1']
    model['vb2'] = -(eps*model['dE_db2']) - momentum*model['vb2']
    model['vb3'] = -(eps*model['dE_db3']) - momentum*model['vb3']
...
def Train(model, forward, backward, update, eps, momentum, num_epochs,
    ...
    for epoch in range(num_epochs):
        ...
        DisplayPlot(train_ce_list, valid_ce_list, 'Cross Entropy',epoch,  number=0)
        DisplayPlot(train_acc_list, valid_acc_list, 'Accuracy',epoch, number=1)
